...
## 고정 UI 규칙 (임의 변경 금지)
- 헤더 메뉴바(감상평/영화 정보 확인하기)는 항상 화면 정중앙에 위치해야 한다.
  `src/components/layout/Header.tsx`의 그리드 구조(주석 참고)와 이를 검증하는
  `src/components/layout/Header.test.tsx` 회귀 테스트를 함께 유지할 것.
  레이아웃을 바꿀 일이 있으면 두 파일을 같이 수정한다.
